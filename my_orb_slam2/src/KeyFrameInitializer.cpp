...
#include "KeyFrameInitializer.h"

namespace ORB_SLAM2
{

	KeyFrameInitializer::KeyFrameInitializer(GlobalData *gd) :
		pVocabulary(gd->mpVocabulary)
	{
		fx = gd->fx;
		fy = gd->fy;
		cx = gd->cx;
		...
		nScaleLevels = gd->nLevels;
		fScaleFactor = gd->fScaleFactor;
		fLogScaleFactor = log(gd->fScaleFactor);
		...
		DistCoef = gd->distCoef;

		if (gd->distCoef.at<float>(0) != 0.0)
		{
			cv::Mat mat(4, 2, CV_32F);
			mat.at<float>(0, 0) = 0.0;
			mat.at<float>(0, 1) = 0.0;
			mat.at<float>(1, 0) = gd->width;
			mat.at<float>(1, 1) = 0.0;
			mat.at<float>(2, 0) = 0.0;
			mat.at<float>(2, 1) = gd->height;
			mat.at<float>(3, 0) = gd->width;
			mat.at<float>(3, 1) = gd->height;

			mat = mat.reshape(2);
			cv::undistortPoints(mat, mat, gd->K, gd->distCoef, cv::Mat(), gd->K);
			mat = mat.reshape(1);
	...
	void KeyFrameInitializer::UndistortKeyPoints()
	{
	...
	bool KeyFrameInitializer::PosInGrid(const cv::KeyPoint & kp, int & posX, int & posY)
	{
	...
	void KeyFrameInitializer::AssignFeaturesToGrid()
	{
