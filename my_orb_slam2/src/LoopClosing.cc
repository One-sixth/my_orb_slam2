...
namespace ORB_SLAM2
{
	...
	LoopClosing::LoopClosing(GlobalData *pGlobalData, Map *pMap, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale) :
		mpMap(pMap), mpGlobalData(pGlobalData), mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mbFixScale(bFixScale)
	{
		mtLoopClosing = thread(&LoopClosing::Run, this);
	}

	LoopClosing::~LoopClosing()
	{
		if (mtLoopClosing.joinable())
			mtLoopClosing.join();
	}
	...
	bool LoopClosing::DetectLoop()
	{
		...
		for (size_t i = 0; i < vpCandidateKFs.size(); ++i)
		{
			...
			for (size_t iG = 0; iG < mvConsistentGroups.size(); ++iG)
			{
				...
				for (set<KeyFrame*>::iterator sit = spCandidateGroup.begin(); sit != spCandidateGroup.end(); ++sit)
				{
	...
	bool LoopClosing::ComputeSim3()
	{
		...
		while (nCandidates > 0 && !bMatch)
		{
			for (int i = 0; i < nInitialCandidates; i++)
			{
				...
				if (!Scm.empty())
				{
					...
					const int nInliers = Optimizer::OptimizeSim3(mpMap, mpCurrentKF, pKF, vpMapPointMatches, gScm, 10, mbFixScale);
		...
		for (vector<KeyFrame*>::iterator vit = vpLoopConnectedKFs.begin(); vit != vpLoopConnectedKFs.end(); vit++)
		{
			...
			if (KeyFrame::isBad(pKF, mpMap))
			{
				cerr << "found bad KeyFrame in LoopClosing::ComputeSim3" << endl;
				continue;
			}
			...
			for (size_t i = 0, iend = vpMapPoints.size(); i < iend; i++)
			{
				...
				if (pMP)
				{
					if (!MapPoint::isBad(pMP, mpMap) && pMP->mnLoopPointForKF != mpCurrentKF->mnId)
					{
	...
	void LoopClosing::SearchAndFuse(const KeyFrameAndPose& CorrectedPosesMap)
	{
		...
		for (KeyFrameAndPose::const_iterator mit = CorrectedPosesMap.begin(); mit != CorrectedPosesMap.end(); ++mit)
		{
