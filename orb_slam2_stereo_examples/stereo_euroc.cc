...
int main(int argc, char **argv1)
{
	...
	char argv[][100] = {
		"",
		R"(..\DBow3-master\orbvoc.dbow3)",
		R"(..\example_data\EuRoC_stereo.yaml)",
		R"(..\example_data\MH_03_medium\mav0\cam0\data)",
		R"(..\example_data\MH_03_medium\mav0\cam1\data)",
		R"(..\example_data\mh_03.txt)"
	};
	...
	for (int ni = 100; ni < nImages; ni+=1)
	{
		...
		cv::remap(imLeft, imLeftRect, M1l, M2l, cv::INTER_CUBIC);
		cv::remap(imRight, imRightRect, M1r, M2r, cv::INTER_CUBIC);
		...
		SLAM.TrackStereo(imLeftRect, imRightRect, /*tframe*/t1.time_since_epoch().count());
	...
	vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
	...
	if (!fTimes.is_open())
	{
		cout << "open timestamp file failure" << endl;
		exit(0);
	}
